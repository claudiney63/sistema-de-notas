...
exports.associateMateriaAndProvideNotas = async (req, res) => {
  try {
    ...
    let materiaIndex = aluno.notas.findIndex(nota => nota.materia.toString() === materiaId.toString());
    if (materiaIndex !== -1) {
      // Se a matéria já estiver associada ao aluno, atualize as notas e faltas
      aluno.notas[materiaIndex].bimestres = notas.map(nota => ({ notas: nota }));
      aluno.notas[materiaIndex].faltas = faltas;
    } else {
      // Se não, crie uma nova instância da matéria para o aluno
      const novaNota = {
        materia: materiaId,
